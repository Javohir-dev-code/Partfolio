...
interface SocialTemplate {
  label: string;
  key: "telegram" | "instagram" | "phone" | "github";
...
export function ProfileCard() {
  ...
  const socialLinks: SocialLinkItem[] = socialTemplates
    .map((s) => {
      const href = s.key === "phone" ? `tel:${settings.phone.replace(/[^\d+]/g, "")}` : settings[s.key];
      if (!href) return null;
      return { label: s.label, href, icon: s.icon };
    })
    .filter((s): s is SocialLinkItem => Boolean(s));
    ...
    <div className="p-6 bg-card rounded-2xl border border-card shadow-[0_1px_2px_0_rgba(26,31,44,0.25)] h-fit">
      ...
      <div className="pb-6">
        ...
        <p className="text-[19px] font-semibold text-p leading-7 mb-2">
          <span className="font-bold text-special">{settings.role || t("profile.role")}</span>{" "}
          {t("profile.desc")}
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 mt-4">
          ...
          >
            ...
            {copied ? t("profile.copied") : t("profile.copyPhone")}
          </button>
        </div>
